...
const AccessType CLIENT_ECHO_MESSAGE		= 0x1;		// si les message doivent se renvoyer a lui meme
const AccessType CLIENT_ACCESS_AWAY 		= 0x2;
const AccessType CLIENT_ACCESS_INVISIBLE	= 0x4;		// +i	| invisible par WHO vu!
const AccessType CLIENT_ACCESS_HIDDEN_HOST	= 0x8;		// +x	| masquage ip: xxx.xxx.xxx.xx => random.username.qwu.org 
const AccessType CLIENT_ACCESS_DEAF 		= 0x10;		// +d	| bloque récéptions message de channels
const AccessType CLIENT_ACCESS_REGISTERED	= 0x20;		// +R	| rejette les message privé
const AccessType CLIENT_ACCESS_WHITELIST	= 0x40;		// +g	| rejette les message privé des personnes à qui tu n'as pas envoyé de message 
const AccessType CLIENT_ACCESS_BOT			= 0x80;		// +B	| indique que l'utilisateur est un bot vu!
const AccessType CLIENT_ACCESS_OPERATOR		= 0x100;	// +oO	| serveur operator vu!
const AccessType CLIENT_ACCESS_ADMIN		= 0x200;	// +aA	| server admin vu!
