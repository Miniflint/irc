...
class Server {
	...
	public:
		...
		bool	handleWho(Client &c, std::istringstream &rest);
		...
		void	handleRplEndOfWho(Client &c, std::string target);
		...
		void	handleErrChannelisfull(Client &c, std::string channelName);
		void	handleErrUnknownmode(Client &c);
		void	handleErrInviteOnlyChan(Client &c, std::string channelName);
